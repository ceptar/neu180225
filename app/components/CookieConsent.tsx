...
export function CookieConsent({
	...
	return variant != "small" ? (
		<div
			className={cn(
				"fixed bottom-0 right-0 z-[200] w-full duration-700 sm:bottom-4 sm:max-w-md",
				!isOpen
		...
		>
			<div className="m-3 border border-border bg-background dark:bg-card">
				...
				<div className="mt-2 flex items-center gap-2 border-t p-3">
					<Button onClick={accept} className="h-9 w-full">
						accept
					</Button>
					<Button onClick={decline} className="h-9 w-full " variant="outline">
						decline
					</Button>
				</div>
			</div>
